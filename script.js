...
.then(function(coun_json){
    ...
    for(var i=0;i<(coun_json.length/3)-1;i++)
    {
        ...
        for(var j=0;j<3;j++)
        {
            ...
            div2.style.backgroundColor="#F26FEE"
            div.appendChild(div2)

            var divhead=document.createElement("div")
            divhead.setAttribute("class","card-header text-center")
            divhead.setAttribute("style","background-color: #41F2DE;")
            ...
            if (coun_json[temp].hasOwnProperty('latlng')) {
                latlng.innerHTML="<b>Latitude and Longitude :</b> "+coun_json[temp].latlng[0]+",&ensp;"+coun_json[temp].latlng[1]
            }
            else {
...
function weather(val)
{
    ...
    .then(function(coun_json){
        ...
        divbody.style.backgroundColor="#7AAEB2"
